...
def scan_line(line, n=4):
    """Return max product of n numbers in a list."""
    largest = 0
    if len(line) < n:
        return -1
...
def scan_grid(grid, n=4):
    """Scan grid for largest product of 4 numbers."""
    largest = 0
    col = []
    ...
    for (i, line) in enumerate(grid[:-(n - 1)]):
        for (j, num) in enumerate(line[:-(n - 1)]):
            value = num
            for k in range(1, n):
                value *= grid[i + k][j + k]
            if value > largest:
