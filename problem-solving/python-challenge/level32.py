...
# Page source 'Fill in the blanks <!-- for warmup.txt -->'
# Download warmup.txt from page http://www.pythonchallenge.com/pc/rock/warmup.txt

# It is a etch-a-sketch puzzle (pic-a-pix)
# Solve warmup.txt is arrow up
# Solve up.txt, result is a snake
# Go to http://www.pythonchallenge.com/pc/rock/python.html
# "Free" as in "Free speech", not as in "free...

# Answer: beer

# Program to solve pic-a-pix
# Code from https://www.hackingnote.com/en/python-challenge-solutions/level-32
...
class Bar:
    def __init__(self, axis, index, segs, length):
        ...
        self.length = length
        self.full_space = sum(
            [seg.length for seg in self.segs]) + len(self.segs) - 1
...
def load():
    with open("./resource/up.txt") as f:
        lines = f.readlines()
...
while True:
    ...
    dirty_bars = [(sum([len(seg.placements) for seg in bar.segs]), bar)
                  for bar in bars if bar.dirty]
    if len(dirty_bars) == 0:
        break
